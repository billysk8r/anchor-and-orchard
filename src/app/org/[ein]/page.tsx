...
export default async function OrgPage({ params }: { params: Promise<{ ein: string }> }) {
  ...
  const [benchmarkRate, irsData] = await Promise.all([getTreasuryRate(), getIrsData(ein)]);
      ...
      {currentBps < 10 && yieldGap > 50 ? (
        <div className="relative overflow-hidden bg-emerald-600 text-white p-12 md:p-16 rounded-[3.5rem] shadow-2xl text-left">
           <div className="relative z-10">
             ...
             <h3 className="text-3xl md:text-4xl font-black mb-6 max-w-2xl leading-tight text-balance">Your cash is currently under-stewarded.</h3>
             <p className="text-emerald-50 text-xl mb-12 max-w-2xl leading-relaxed opacity-90">By shifting liquid assets to a strategy matching the {(benchmarkRate * 100).toFixed(2)}% treasury benchmark, this organization could fund more of its mission.</p>
             <div className="flex flex-col md:flex-row items-start md:items-center gap-10">
               <div className="bg-white text-emerald-700 px-12 py-8 rounded-[2rem] shadow-inner">
                 ...
                 <p className="text-5xl font-black tracking-tighter">+${Math.floor(yieldGap).toLocaleString()}</p>
               </div>
               {impactText && <div className="max-w-[250px] text-emerald-100 text-sm leading-relaxed border-l border-emerald-500/50 pl-8 italic">{impactText}</div>}
             </div>
           </div>
